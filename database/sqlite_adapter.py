...
def _run_migrations(conn: sqlite3.Connection) -> None:
    # Apply every ordered migration script present in the migrations directory.
    migration_dir = Path(__file__).parent / "migrations"
    if migration_dir.is_dir():
        for migration_file in sorted(migration_dir.glob("0*_*.sql")):
...
# --- Hermosa Connect: congestion + hotspots ---


def insert_congestion_observation(
    camera_id: int | None,
    location: str | None,
    obs_time: str,
    *,
    ...
    with db_session() as conn:
        cursor = conn.execute(
            ...
            INSERT INTO congestion_observations
                (camera_id, location, obs_time, vehicle_count, moving_count,
                 stationary_count, avg_speed_kmh, density_score, source)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                camera_id,
...
def insert_congestion_event(
    ...
    *,
    ...
    avg_density_score: float | None = None,
) -> int:
    with db_session() as conn:
        cursor = conn.execute(
            """
            INSERT INTO congestion_events
                (camera_id, location, severity, state, started_at, ended_at,
                 duration_minutes, peak_vehicle_count, avg_density_score)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                camera_id,
...
def list_congestion_events(limit: int = 100, camera_id: int | None = None) -> list[dict[str, Any]]:
...
def replace_hotspots(rows: list[dict[str, Any]]) -> None:
    """Replace the hotspot table with a freshly computed ranking."""
    with db_session() as conn:
        ...
        for rank, row in enumerate(rows, start=1):
            conn.execute(
                ...
                INSERT INTO hotspots
                    (location, camera_id, frequency_score, avg_duration_minutes,
                     peak_severity, rank)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
...
def list_hotspots(limit: int = 50) -> list[dict[str, Any]]:
    with db_session() as conn:
        rows = conn.execute(
            "SELECT * FROM hotspots ORDER BY rank ASC LIMIT ?", (limit,)
        ).fetchall()
