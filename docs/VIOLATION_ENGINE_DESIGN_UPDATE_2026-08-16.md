...
**Vehicle detector labels (supersession):** Any earlier wording that treated `suv_crossover`, `uv_express_van`, or `piaggio` as canonical YOLO vehicle detector classes is **superseded** by `config/training/class_schema.json` schema_version **1.3.0** (10-class roster: SUV/crossover consolidated into `car`; single `van`; `autorickshaw` by body form; Piaggio brand metadata only; UV Express / for-hire as contextual metadata). See `docs/VEHICLE_CLASSIFICATION_SPECIFICATION.md` and §A.3 Illegal Terminal below.
